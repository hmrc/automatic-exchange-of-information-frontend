  ...
  "signOut" - {
    ...
    "must clear user answers and redirect to sign out, specifying the exit survey as the continue URL" in {
      ...
      running(application) {
        ...
        val request   = FakeRequest(GET, routes.AuthController.signOut(serviceMDR).url)
  ...
  "signOutNoSurvey" - {
    ...
    "must clear users answers and redirect to sign out, specifying SignedOut as the continue URL" in {
      ...
      running(application) {
        ...
        val appConfig = application.injector.instanceOf[FrontendAppConfig]
        val request   = FakeRequest(GET, routes.AuthController.signOutNoSurvey(serviceMDR).url)

        val result = route(application, request).value

        val encodedContinueUrl  = URLEncoder.encode(routes.SignedOutController.onPageLoad(serviceMDR).url, "UTF-8")
        val expectedRedirectUrl = s"${appConfig.signOutUrl}?continue=$encodedContinueUrl"
